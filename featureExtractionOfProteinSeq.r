...
##########################################
# Feature 1: aaComp | Feature Count = 9
##########################################
F1 = unlist(aaComp(sequence))[10:18]
names(F1) <- paste("F1_aaComp_",c(1:9),sep='')


#######################################################
# Feature 2: aaDescriptors | Feature Count = 53*4=212
#######################################################
F2     <- matrix(aaDescriptors(sequence), nrow = 53, byrow = TRUE)
F2_Min <- apply(F2, 1, function(x) min(x[x != 9]))
F2_Max <- apply(F2, 1, function(x) max(x[x != 9]))
F2_Mean<- apply(F2, 1, function(x) mean(x[x != 9]))
F2_Sd  <- apply(F2, 1, function(x) sd(x[x != 9]))
F2     <- c(F2_Min,F2_Max,F2_Mean,F2_Sd)
names(F2)<- paste("F2_aaDescriptors_",c(1:212),sep='')


#######################################################
# Feature 3: aIndex | Feature Count = 1
#######################################################
F3 <- aIndex(sequence)


#######################################################
# Feature 4: autoCorrelation | Feature Count = 1
#######################################################
sequence = "SDKEVDEVDAALSDLEITLE"
sequence = "SDKEVDEVDAA"
...
F3 <- matrix(F3, nrow = nchar(sequence), byrow = TRUE)
...
mydata = read.table("peptides.csv")

#sequence<-readDNAStringSet("peptides.fasta")
i=0
for (peptide in mydata$V1){
  i=i+1
  cat("\n Completed: ",i,"/", nrow(mydata))
